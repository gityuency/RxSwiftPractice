...
class Vc_3_In_One: UIViewController {
    
    deinit {
        print("销毁 Vc_3_In_One")
    }
    ...
    func usage_1() {
        ...
        observable.subscribe { (event) in
            print(event.element)  //  这个能得到数据, 但是在 completed 里面拿到的数据是 nil
        }
    }
    ...
    func usage_4() {
        ...
        observable.subscribe { event in
            print(event)  // 这里仍然会有输出,因为订阅了, 如果这个 observable 发出事件呢?
        }
    }
